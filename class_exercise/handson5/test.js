...
function produceAndPositionScatterPlot(data1, data2, field1, field2) {
    ...
    if (p_value <= p_threshold) {
        // Put the chart in the significant column
        container_id += "significant";
    }
    else {
        // Put the chart in the insignificant column
        container_id = container_id + "insignificant";
    }

    // Create an SVG element in the right column.
...
function renderScatterplot(svg, field1, data1, field2, data2) {
    ...
    // Create a group to transform graphics by the margins.
    ...
    chart_g.selectAll(".dot").data(zipped_data).enter().append('circle')
        .attr('class', 'dot')
